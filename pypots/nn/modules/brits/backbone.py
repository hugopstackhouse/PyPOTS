...
class BackboneRITSI(nn.Module):
    """model RITSI: (Uncorrelated) Recurrent Imputation for Time Series
    ...
    def forward(self, inputs: dict, direction: str) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for t in range(self.n_steps):
            ...
            # c_c = m * x + (1 - m) * c_h
            estimations.append(x_h.unsqueeze(dim=1))

            inputs = torch.cat([x_c, m], dim=1)
            hidden_states, cell_states = self.rnn_cell(inputs, (hidden_states, cell_states))

        reconstruction_loss /= self.n_steps 
...
class BackboneBRITSI(nn.Module):
    """model BRITSI: Bidirectional RITSI
    BRITSI consists of two RITSI, which take time-series data from two directions (forward/backward) respectively. Features at each time are assumed to be uncorrelated.
    ...
    ritsi_f: RITSI object
        the forward RITSI model

    ritsi_b: RITSI object
        the backward RITSI model
    ...
    def __init__(
        ...
        self.rnn_hidden_size = rnn_hidden_size
        # create models
        self.ritsi_f = BackboneRITSI(n_steps, n_features, rnn_hidden_size)
        self.ritsi_b = BackboneRITSI(n_steps, n_features, rnn_hidden_size)
    ...
    def forward(self, inputs: dict) -> Tuple[torch.Tensor, ...]:
        # Results from the forward RITSI.
        (
            f_imputed_data,
            f_reconstruction,
            f_hidden_states,
            f_reconstruction_loss,
        ) = self.ritsi_f(inputs, "forward")
        # Results from the backward RITSI.
        (
            b_imputed_data,
            b_reconstruction,
            b_hidden_states,
            b_reconstruction_loss,
        ) = self._reverse(self.ritsi_b(inputs, "backward"))
