...
def evaluate_fold(model, fold, Xs, ys, fold_idxs, windows):
    # make the splits
    ...
    # loop over % missing
    errs = np.zeros((len(windows), X_test_fold.shape[0]))
    for (idx, widx) in enumerate(windows):
        ...
        errs[idx, :] = np.fromiter(map(calc_mae, imputed, X_test_fold, mask), dtype=np.float64) # get individual errors for uncertainty quantification

    return errs

def evaluate_folds(model, nfolds, Xs, ys, fold_idxs, windows):
    errs = []
    for fold in range(nfolds):
        print(f"Evaluating fold {fold}/{nfolds}...")
        errs.append(evaluate_fold(model, fold, Xs, ys, fold_idxs, windows))
...
fold_scores_britsi = evaluate_folds(britsi, 3, Xs, ys, resample_fold_idxs, [range(5,15)])

print("IPD BRITS Mean MAE: {}".format(np.mean(fold_scores_brits)))
print("IPD BRITS-I Mean MAE: {}".format(np.mean(fold_scores_britsi)))